...
### Database Patterns
- Use SQLAlchemy ORM patterns established in `feedback_models.py`
...
1. Add route to `feedback_app.py`
...
1. Update model in `feedback_models.py` (Person, Feedback, or ManagerFeedback)
...
1. Create HTML template in `feedback_templates/` directory
2. Extend appropriate base (or create standalone)
3. Add route in `feedback_app.py` to render template
...
- **Template**: `tenets-sample.json` (version controlled)
- **Structure**:
  ```json
  {
    "version": "2025-Q4",
...
- Files generated by `create_sample_feedback_data.py`
- Sample tenets: `tenets-sample.json`
...
- **Do not add**: Database models (belongs in feedback_models.py)

#### `feedback_models.py` (Database Schema)
...
- **Do not add**: Business logic or route handlers (belongs in feedback_app.py)
...
#### `feedback_templates/individual_feedback.html` (Feedback Collection)
...
#### `feedback_templates/export_list.html` (Export Manager List)
- List of managers with feedback counts
- Download links per manager
- Shows associates with pending feedback

#### `feedback_templates/manager_select.html` (Manager Login)
...
- Created by copying and modifying tenets-sample.json
...
**Solution**: Copy `tenets-sample.json` to `tenets.json` and customize
